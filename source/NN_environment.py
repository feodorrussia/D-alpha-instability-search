...
def clear_short_fragments(data: np.array, edge: int) -> np.array:
    # edge = 50
...
def get_prediction_unet(data: np.array, POINTS_DIM = 1024, ckpt_v=0) -> np.array:
    ...
    while l_edge + POINTS_DIM < new_df.shape[0]:
        predictions = model.predict(normalise_series(data[l_edge:l_edge + POINTS_DIM - 1]), verbose=0)
        for i in range(0, POINTS_DIM):
            prediction_result[l_edge + i] = predictions[0][i]
        l_edge += step
    
    if l_edge + POINTS_DIM - step != new_df.shape[0] - 1:
        predictions = model.predict(normalise_series(data[data.shape[0] - POINTS_DIM:]), verbose=0)
        for i in range(0, POINTS_DIM):
            prediction_result[new_df.shape[0] - POINTS_DIM + i] = predictions[0][i]

    prediction_result = down_to_zero(prediction_result, edge=0.3)
    prediction_result = clear_short_fragments(prediction_result, edge=10)
